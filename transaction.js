...
try {
  ...
  const book = library.books.findOne({_id:'B041','copies.available':{$gt:0}});
  if (!book) throw new Error('Book is unavailable.');

  library.loans.insertOne({
    _id:'L021', book_id:'B041', member_id:'M001',
    issue_date:new Date(),
    due_date:new Date(Date.now()+14*24*60*60*1000),
    return_date:null, status:'Issued', fine:0
  });
...
db.books.findOne({_id:'B041'},{title:1,copies:1});
